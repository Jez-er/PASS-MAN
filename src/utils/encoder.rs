...
use cipher::{generic_array::GenericArray, BlockSizeUser, KeyInit, BlockCipher};
...
pub fn encrypt_password(password: &str, key: &[u8], iv: &[u8]) -> Result<String, &'static str> {
    ...
    Ok(encode(&padded)) // Returning a Base64 representation
}
...
pub fn decrypt_password(encrypted: &str, key: &[u8], iv: &[u8]) -> Result<String, &'static str> {
    ...
    let cipher = Aes256::new(GenericArray::from_slice(key));
    let mut encrypted_bytes = decode(encrypted).map_err(|_| "ERROR | Failed to decode Base64")?;
...
// Password comparison
pub fn compare_passwords(input_password: &str, encrypted_password: &str, key: &[u8], iv: &[u8]) -> bool {
    match decrypt_password(encrypted_password, key, iv) {
        Ok(decrypted_password) => decrypted_password == input_password,
        Err(_) => false,
    }
}
